...
import {
    ...
    ChevronLeft, ChevronRight, Upload // <--- Added Upload Icon
} from 'lucide-react';
import { getAuth, signOut, updateProfile } from "firebase/auth";
import { getFirestore, doc, setDoc } from "firebase/firestore";
import { getStorage, ref, uploadBytes, getDownloadURL } from "firebase/storage"; // <--- Added Firebase Storage
...
const COUNTRIES = [
    ...
    { code: "IN", name: "India" }, { code: "ID", name: "Indonesia" }, { code: "IT", name: "Italy" },
    ...
    { code: "PK", name: "Pakistan" }, { code: "PH", name: "Philippines" }, { code: "RU", name: "Russia" },
    { code: "SA", name: "Saudi Arabia" }, { code: "ZA", name: "South Africa" }, { code: "ES", name: "Spain" },
...
const Profile = ({ user, onLogin }) => {
    ...
    // --- NEW: CUSTOM FILE UPLOAD HANDLER ---
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            // Saves the image securely in Firebase Storage under their user ID
            const avatarRef = ref(storage, `avatars/${user.uid}/profile_pic`);
            ...
            alert("Upload failed! Ensure Firebase Storage is enabled in your Firebase Console.");
            setSaving(false);
        }
    };
                ...
                <div className="fixed inset-0 z-[600] bg-[#020617]/95 backdrop-blur-2xl flex items-center justify-center p-6 animate-in fade-in">
                    <div className="glass-panel p-8 md:p-12 rounded-[3rem] max-w-xl w-full border-white/10 relative shadow-2xl bg-[#0a0a0a] overflow-hidden">
                        ...
                        <div className="mb-8 relative group">
                            ...
                            />
                            <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-white/10 rounded-3xl group-hover:border-neon-cyan/50 group-hover:bg-neon-cyan/5 transition-all">
                                <Upload size={32} className="text-zinc-500 group-hover:text-neon-cyan mb-3 transition-colors" />
                                <span className="text-[12px] font-black uppercase tracking-widest text-zinc-400 group-hover:text-white transition-colors">
                                    Upload Custom Image
                                </span>
                                <span className="text-[9px] text-zinc-600 uppercase tracking-widest mt-2">Max Size: 5MB</span>
                            </div>
                        </div>
