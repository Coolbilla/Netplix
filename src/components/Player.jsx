...
import {
    ...
    ChevronRight, ChevronLeft, ShieldAlert, MonitorPlay, CheckCircle
} from 'lucide-react';
...
const Player = ({ media, toggleCompleted, onClose }) => {
    ...
    if (showPostPlay) {
        return (
            <div className="fixed inset-0 z-[6000] bg-[#020202] flex items-center justify-center animate-in fade-in zoom-in-95 duration-500 font-sans p-6">
    ...
    return (
        <div className="fixed inset-0 z-[5000] bg-[#020202] flex flex-col animate-in fade-in duration-500 overflow-hidden font-sans">
            ...
            <div className="absolute top-0 left-0 w-full p-4 md:p-8 z-[5010] flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pointer-events-none">
                ...
                <div className="pointer-events-auto flex items-center bg-black/40 backdrop-blur-xl border border-white/10 p-1.5 rounded-full shadow-2xl overflow-x-auto max-w-full no-scrollbar">
                    <div className="flex items-center px-3 gap-2 border-r border-white/10 mr-1 hidden md:flex">
                        <MonitorPlay size={16} className="text-zinc-400" />
                        <span className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Source</span>
                    </div>
                    {['cinezo', 'vidsrc', 'vidora'].map((server) => (
                        <button
                            key={server}
                            onClick={() => setActiveServer(server)}
                            className={`relative px-4 py-2 md:py-2.5 rounded-full font-black text-[10px] md:text-[11px] uppercase tracking-wider transition-all duration-300 shrink-0 ${activeServer === server ? 'text-black bg-neon-cyan shadow-[0_0_20px_rgba(0,255,255,0.3)]' : 'text-zinc-400 hover:text-white hover:bg-white/10'}`}
                        >
                            {server === 'cinezo' ? 'Server 1 (No Ads)' : server === 'vidsrc' ? 'Server 2 (Fast)' : 'Server 3'}
                        </button>
                    ))}
                </div>
            </div>

            <div className="flex-1 w-full h-full relative z-[5005] md:p-12 md:pt-28 pb-0 flex items-center justify-center">
                <div className="relative w-full h-full max-w-[1600px] mx-auto md:rounded-[2rem] overflow-hidden bg-black md:border border-white/10 md:shadow-[0_0_60px_rgba(0,0,0,0.8)] group">
                    <iframe key={streamUrl} src={streamUrl} className="w-full h-full border-none absolute inset-0" allowFullScreen allow="autoplay; encrypted-media" scrolling="no" title={`${activeServer} Stream`} />
            ...
            {showAdWarning && (
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-[5020] bg-black/80 backdrop-blur-xl border border-white/10 text-white pl-4 pr-2 py-2 rounded-full flex items-center gap-4 shadow-2xl animate-in slide-in-from-bottom-5 fade-in duration-500 pointer-events-auto w-max max-w-[90vw]">
