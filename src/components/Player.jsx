...
import { MediaErrorHandler, ErrorTypes } from '../utils/errorHandling'; // <-- IMPORTED ENGINE
import {
    Play, Info, Zap, Star, Calendar, ShieldCheck, Flame, X, Cpu, Shield,
    ChevronRight, ChevronLeft, ShieldAlert, MonitorPlay, CheckCircle, ChevronDown, RefreshCw
} from 'lucide-react';
...
const Player = ({ media, toggleCompleted, onClose }) => {
    ...
    // --- NEW RECOVERY STATES ---
    const [healthStatus, setHealthStatus] = useState('loading'); // loading, stable, degraded
    ...
    // If the user is on the page for 12 seconds and hasn't changed servers, 
    // we ask them if the stream is working.
    useEffect(() => {
        setHealthStatus('loading');
        const monitorTimer = setTimeout(() => {
            setHealthStatus('stable'); // Assume stable, but show the fix button just in case
        }, 12000);
    ...
    const vidsrcUrl = type === 'tv' ? `https://vidsrc-embed.ru/embed/${tmdbId}/${season}-${episode}` : `https://vidsrc-embed.ru/embed/${tmdbId}`;
    ...
    const serverOptions = [
        ...
        { id: '111movies', name: 'Server 4 (Backup)' }
    ];
    ...
    // --- ENTERPRISE RECOVERY TRIGGER ---
    const triggerAutoRecovery = () => {
        const errorHandler = new MediaErrorHandler(
            serverOptions,
            activeServer,
            (newServerId) => setActiveServer(newServerId), // Callback to switch server
            (msg, type) => showNotification(msg, type)     // Callback to show toast
        );

        // Tell the engine to execute a failover
        errorHandler.triggerFailover(ErrorTypes.IFRAME_LOAD_ERROR);
    };
        ...
        return (
            <div className="fixed inset-0 z- bg-[#020202] flex items-center justify-center animate-in fade-in zoom-in-95 duration-500 font-sans p-6">
                ...
                <div className="relative z-10 max-w-2xl w-full bg-black/60 backdrop-blur-3xl border border-white/10 p-8 md:p-12 rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,1)] text-center flex flex-col items-center">
                    ...
                    <p className="text-zinc-400 text-sm md:text-base font-medium mb-10 max-w-md">Did you finish <span className="text-white">"{media.title || media.name}"</span>? Mark it as watched or keep it in your active history.</p>
                    <div className="flex flex-col md:flex-row gap-4 w-full justify-center">
                        <button onClick={handleMarkAsCompleted} className="bg-yellow-500 hover:bg-yellow-400 text-black px-8 py-5 rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-[0_0_30px_rgba(234,179,8,0.4)] flex items-center justify-center gap-3 hover:scale-105 active:scale-95"><CheckCircle size={20} /> Mark as Watched</button>
                        <button onClick={onClose} className="glass-panel hover:bg-white/10 text-white px-8 py-5 rounded-2xl font-black uppercase tracking-widest text-xs transition-all border border-white/10 flex items-center justify-center gap-3 hover:scale-105 active:scale-95">Just Close</button>
                    </div>
                    <button onClick={() => setShowPostPlay(false)} className="absolute top-6 left-6 text-zinc-500 hover:text-white flex items-center gap-2 text-[10px] uppercase font-black tracking-widest transition-colors"><ChevronLeft size={16} /> Back to Video</button>
                </div>
            </div>
        );
    }
        ...
        <div className="fixed inset-0 z- bg-[#020202] flex flex-col animate-in fade-in duration-500 overflow-hidden font-sans">
            ...
            <div className="absolute top-0 left-0 w-full p-4 md:p-8 z-50 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pointer-events-none">
                ...
                <div className="pointer-events-auto flex items-center gap-4 bg-black/40 backdrop-blur-xl border border-white/10 p-2 pr-6 rounded-full shadow-2xl">
                    <button onClick={() => setShowPostPlay(true)} className="bg-white/10 hover:bg-red-500/80 text-white p-3 rounded-full transition-all duration-300 group cursor-pointer">
                        <X className="w-6 h-6 group-hover:rotate-90 transition-transform duration-300" />
                    </button>
                    <div className="flex flex-col justify-center">
                        <span className="text-[9px] md:text-[10px] font-black text-neon-cyan uppercase tracking-[0.3em]">{type === 'tv' ? 'Series Engine' : 'Cinema Engine'}</span>
                        <h2 className="text-white font-bold text-sm md:text-base tracking-wide max-w-[200px] md:max-w-md truncate">{type === 'tv' ? `Season ${season} • Episode ${episode}` : media.title || media.name}</h2>
                    </div>
                </div>

                {/* Right Side: Recovery & Source Dropdown */}
                <div className="pointer-events-auto flex flex-wrap items-center justify-end gap-3" ref={dropdownRef}>

                    {/* AUTO RECOVERY BUTTON (Shows after 12s) */}
                    {healthStatus === 'stable' && (
                        <button
                            onClick={triggerAutoRecovery}
                            className="flex items-center gap-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 px-4 py-3 rounded-full transition-all active:scale-95 animate-in fade-in duration-1000 shadow-[0_0_15px_rgba(239,68,68,0.15)] cursor-pointer"
                        >
                            <RefreshCw size={14} className="animate-spin-slow" />
                            <span className="text-[9px] font-black uppercase tracking-widest">Auto-Fix Stream</span>
                        </button>
                    )}

                    {/* SOURCE DROPDOWN */}
                    <div className="relative">
                        <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-3 bg-black/40 backdrop-blur-xl border border-white/10 px-5 py-3 rounded-full shadow-2xl hover:bg-white/5 transition-colors cursor-pointer">
                            ...
                            <div className="flex flex-col items-start text-left">
                                ...
                                <span className="text-xs font-bold text-white leading-none">{serverOptions.find(s => s.id === activeServer)?.name}</span>
                            </div>
                        ...
                        {isDropdownOpen && (
                            <div className="absolute right-0 top-full mt-2 w-full min-w-[200px] bg-[#0a0a0a]/95 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.8)] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                {serverOptions.map((server) => (
                                    <button
                                        key={server.id}
                                        onClick={() => { setActiveServer(server.id); setIsDropdownOpen(false); }}
                                        className={`w-full text-left px-5 py-4 text-xs font-bold transition-all flex items-center justify-between border-b border-white/5 last:border-0 hover:bg-white/5 cursor-pointer ${activeServer === server.id ? 'text-neon-cyan bg-neon-cyan/5' : 'text-zinc-300'}`}
                                    >
            ...
            {systemNotification && (
                <div className="absolute top-24 right-4 md:right-8 z- animate-in slide-in-from-right-8 fade-in duration-300">
                    <div className={`glass-panel border px-5 py-3 rounded-xl flex items-center gap-3 shadow-2xl ${systemNotification.type === 'fatal' ? 'border-red-500/50 bg-red-500/10' : 'border-yellow-500/50 bg-yellow-500/10'
                        }`}>
                        {systemNotification.type === 'fatal' ? <X size={16} className="text-red-500" /> : <Zap size={16} className="text-yellow-500 animate-pulse" />}
                        <span className="text-[10px] font-black uppercase tracking-widest text-white">{systemNotification.message}</span>
                    </div>
                </div>
            )}
